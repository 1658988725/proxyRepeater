...
int main(int argc, char** argv) {
		...
		case 'c':
			...
			if (!conf) {
				*env << "File not found or json parse fail.\"" << optarg << "\"\n";
				...
				/*
				for (CP_ARRAY::iterator it = channels.begin(); it != channels.end(); ++it) {
					openURL(*env, (*it).rtspURL, (*it).rtmpURL, (*it).rtspUseTcp, (*it).rtspAUTH);
				}
				*/
				cJSON_Delete(conf);
				conf = NULL;
	...
	DONE: env->taskScheduler().doEventLoop(&eventLoopWatchVariable);
	return 0;
}
...
ourRTSPClient::ourRTSPClient(UsageEnvironment& env, unsigned channelId)
	: RTSPClient(env, channels[channelId].rtspURL, RTSP_CLIENT_VERBOSITY_LEVEL, progName, 0, -1),
